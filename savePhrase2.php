<?php
    ...
    //print_r($taps);
    $insert_query = "INSERT INTO experiment1(SessionID,SessionDate,dependentVariable,phraseNumber,language,phraseShown,phraseTyped,editdistance,typingTime,keystrokes,backspaces) VALUES ";
    $insert_body ="";
    ...
    for($i=0;$i<count($taps);$i++){
    	...
    	$insert_body.="('".$id."','".$currentDate."','".$dependentVariable."',".$taps[$i]->phraseSequenceNumber.",'".$taps[$i]->phraseLanguage."','".$taps[$i]->phraseShown."','".$taps[$i]->phraseTyped."',".$taps[$i]->editdistance.",".$taps[$i]->timeTaken.",'".$taps[$i]->ksLogs."',".$backpresses[$i].")";
