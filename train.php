...
<html>
	...
	<script type="text/javascript">
			...
			function onLoad()
			{
                ...
                if(currentsesh>0 && currentsesh!=session)
				{
					console.log("I am in if section of onLoad function");
			// 		<form autocomplete="off">
			// 	<label id="toType"></label>
			// 	<br/>
			// 	<input type="text" onkeyup="calculate()" id="tt" onKeyPress="return disableEnterKey(event)" autocomplete="off" autofocus />
			...
			function syncToServer()
			{
				...
				{
					onLoad();
					saveDependentVariable('swarachakra');
					currentsesh++;
				}
				var xmlhttp;
				...
				var url = "savePhrase2.php";
				var params ="id="+uCode+"_"+session+"_"+uniqueIdentifier+"&var="+dependentVariable+"&phrases=["+tapLogsArray+"]"+"&backspace=["+backarray+"]";
			...
			{
               console.log("Hello i reached the load phrases function after session 1");
				var xmlhttp;
...
function myshuffle(sourceArray) {
    ...
    return sourceArray;
        ...
        if (toTypeSequenceArray.length > 0) {
            ...
            // Check if the "toType" element exists before trying to set its innerHTML
            var toTypeElement = document.getElementById("toType");
            if (toTypeElement) {
                toTypeElement.innerHTML = toTypeArray[toTypeArrayIndex];
            }
        }
    ...
    } else 
	{
        ...
        if (toTypeArrayIndex < toTypeArray.length) {
            ...
            // If no more phrases, reset variables and update HTML elements
            document.getElementById("container").innerHTML = '<span class="span2">Done. Starting new session...</span>';
