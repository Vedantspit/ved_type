...
<body onload="onLoad()">
		<div>
			<div id="buttoncontainer">
				...
				<fieldset id="study_info">
                        ...
                        <label>No. of Phrases per session </label><input type="text" id="nmph"><br />
                        <label>Repetition Factor</label><input type="text" id="rf"><br />
                        Select keyboard	
                         <br/>
    ...
    var xl=0;
         ...
         var sessionPhrases = {};
         var phraseCount = {};
         toTypeSequenceArray = shuffleArray(toTypeSequenceArray);
         console.log("hi")
         	...
         	xmlhttp.onreadystatechange = function() {
         	    ...
         	    if (this.readyState == 4 && this.status == 200) {
                    ...
                    console.log(trainingWords);
         	         shuffleArray(trainingWords)
                    console.log(trainingWords);
                    console.log("TOTAL LENGTH OF PHRASE_SET is "+trainingWords.length);
         		
         		
         		    trainingWords = shuffleArray(trainingWords);
                     console.log(trainingWords);
         ...
         function dividePhrases(numSessions,phrasesPerSession,repetitionFactor)
        {
         ...
         for (let session = 1; session <= numSessions; session++) {
           const availablePhrases = trainingWords.slice(); // Create a copy to avoid modifying the original array
        ...
        return sessionPhrases;    
         ...
         function saveDependentVariable(dVariable){
            ...
            repeatfactor=document.getElementById("rf").value;
			console.log("no. of phrases are "+nmph+"no. of sessions are "+session+" and Repetiton factor is "+repeatfactor+" participant id is "+uCode);
         	...
         	if(navigator.onLine == true){
                ...
                if(currsesh<=session)
                {
                    const root = builder.create('sessions');
          
         		toTypeArray = dividePhrases(session,nmph,repeatfactor);
                document.getElementById("toType").innerHTML = toTypeArray[currsesh+1][0];
                //  toTypeArray=shuffleArray(toTypeArray);
                }
         ...
         function showNextPhrase()
         {
			...
			// calcCorrected();
            if(toTypeArrayIndex <toTypeArray[currsesh+1].length)
            {
			infarray.push(inf);
			farray.push(fix);
			ifcarray.push(ifc);
            ...
            count=0;
         	...
         	var toType = "";
         	var typed = "";
            console.log("TO TYPE ARRAY LENGTH IS "+toTypeArray[currsesh+1].length)
         	var phrase = {};
             if (toTypeArrayIndex > toTypeArray[currsesh+1].length) 
             {
        ...
        toType = toTypeArray[currsesh+1][toTypeArrayIndex-1];
         ...
         */
        if((toTypeArrayIndex>=toTypeArray[currsesh+1].length) && (currsesh==session))  
        {
            document.getElementById("tt").innerHTML = "";
     ...
     }else
     {
        if(currsesh==session)
         ...
         if((toTypeArrayIndex+1)<=toTypeArray[currsesh+1].length ){
       ...
       {
         document.getElementById("toType").innerHTML = toTypeArray[currsesh+1][toTypeArrayIndex];
       }
